...
function statusClass(status) {
  const map = {
    ...
    'Casual':      'badge-blue',
    'Contractual': 'badge-orange',
    'Job Order':   'badge-gray',
    'Co-terminus': 'badge-purple',
  }
  ...
  <div class="page">
    ...
    <div class="toolbar">
      <div class="toolbar-left">
        ...
        <AppSelect
          v-model="filterDept"
          :options="[{ label: 'All Departments', value: '' }, ...store.departments.map(d => ({ label: d, value: d }))]"
          placeholder="All Departments"
          @update:modelValue="resetPage"
        />
        <AppSelect
          v-model="filterStatus"
          :options="[{ label: 'All Status', value: '' }, ...store.employmentStatuses.map(s => ({ label: s, value: s }))]"
          placeholder="All Status"
          @update:modelValue="resetPage"
        />
        <AppSelect
          v-model="filterGender"
          :options="[{ label: 'All Gender', value: '' }, { label: 'Male', value: 'Male' }, { label: 'Female', value: 'Female' }]"
          placeholder="All Gender"
          @update:modelValue="resetPage"
        />
        <AppSelect
          v-model="filterGroup"
          :options="[{ label: 'All Groups', value: '' }, { label: 'KP', value: 'KP' }, { label: 'GEAMH', value: 'GEAMH' }]"
          ...
          <tr v-for="emp in filtered" :key="emp.id">
            ...
            <td>
              <div class="action-btns">
                <button class="btn-icon" title="Edit" @click="router.push(`/employees/${emp.id}/edit`)">
...
.btn-icon {
  ...
  border-radius: 4px; transition: background 0.2s; display: inline-flex; align-items: center;
}
...
@media (max-width: 768px) {
  ...
  .emp-avatar { display: none; }
  .table-wrapper { max-height: calc(100vh - 220px); }
